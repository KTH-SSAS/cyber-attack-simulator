...
class Net(Module):
    def __init__(
        ...
        self.opt = torch.optim.AdamW(self.parameters(), lr=opt_lr, weight_decay=opt_l2)
    ...
    def update(self, r, v, pi, s_, num_obj, done, target_net=None):
        ...
        norm = torch.nn.utils.clip_grad_norm_(self.parameters(), self.opt_max_norm)
        self.opt.step()
...
class MultiMessagePassing(Module):
    def __init__(
        ...
        self.gnns = ModuleList(
            [
                GraphNet(node_in_size, edge_size, global_size, agg_size, node_out_size)
...
class GraphNet(MessagePassing):
    def __init__(self, node_in_size, edge_size, global_size, agg_size, node_out_size):
        ...
        self.f_agg = Sequential(
            Linear(node_in_size + global_size + agg_size, node_out_size), LeakyReLU()
        )
